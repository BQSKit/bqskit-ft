...
class DPFPass(BasePass):
    ...
    def choose_best_circuit(self, 
        ...
        Ts = 1
        RZs = -1 * log10(success_threshold) * 10
        ...
        def cost_fn(circ: Circuit) -> float:
            ...
            rz_cost_per_gate = np.ceil(-1 * np.log10(success_threshold)) * 10
            return num_t + num_rz * rz_cost_per_gate + frac_z_cost

        best_circuits = sorted(circuits, key=cost_fn)[:num]
        return best_circuits      
        
    async def run(self, circuit: Circuit, data: PassData) -> None:
        prev_circs: list[Circuit] = data.get('prev_ntro_circs', [circuit.copy()])

        # Get success threshold from algorithmic_error
        success_threshold = None
        for key in data.keys():
            if 'algorithmic_error' in key:
                success_threshold = data[key]
                break

        if success_threshold is None:
            raise ValueError("No algorithmic_error found in PassData for DPFPass.")
        ...
        # Choose best 3 circuits from new_circs
        best_circs = self.choose_best_circuit(new_circs, 
                                              success_threshold, 
                                              num=3)
        prev_circs.extend(best_circs)

        best_circ = best_circs[0]
